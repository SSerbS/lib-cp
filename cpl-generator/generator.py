...
def draw_algorithms(pdf, library_data, lexer, save_toc=False):
    ...
    for category_name, files in sorted(library_data.items()):
        # 1. Ordena os arquivos primeiro
        sorted_files = sorted(files, key=lambda x: x['name'])
        
        # 2. Descobre a altura exata do PRIMEIRO arquivo desta categoria
        first_file_lines = sorted_files[0]['content'].count('\n') + 1
        first_file_height = 12 + (first_file_lines * 3.0)
        
        # 3. Exige espaço junto: Título (10) + Primeiro Código
        pdf.check_space(10 + min(first_file_height, 270))
        ...
        for item in sorted_files:
            # Mantemos a proteção para não rasgar as caixinhas no meio!
            num_lines = item['content'].count('\n') + 1
            estimated_height = 12 + (num_lines * 3.0) 
            
            if estimated_height < 275:
                pdf.check_space(estimated_height)
            else:
                pdf.check_space(12)
    ...
    for category_name, files in sorted(library_data.items()):
        # Exige 40 de espaço (o suficiente para o título + um pedaço do primeiro código)
        pdf.check_space(40) 
        pdf.ln(2)
        pdf.set_font(pdf.code_font, 'B', 10)
        pdf.set_text_color(220, 50, 47)
    ...
    for category_name, files in sorted(library_data.items()):
        ...
        # 3. Exige espaço junto: Título (10) + Primeiro Código
        # (Usamos min(..., 270) para evitar bugs caso o arquivo seja maior que a folha)
        pdf.check_space(10 + min(first_file_height, 270))
        ...
        for item in sorted_files:
            ...
            pdf.start_box()
            # (Não precisa alterar nada daqui para baixo, o resto continua igual...)
            pdf.set_font(pdf.code_font, 'B', 7.5)
            pdf.set_fill_color(0, 0, 0)
            pdf.set_text_color(255, 255, 255)
            pdf.cell(pdf.col_width, 5, f" {item['name']}", fill=True, new_x='LMARGIN', new_y='NEXT')
...
if __name__ == '__main__':
    ...
    if lib_data:
        user_info = {
            'name': 'SSerbS, GuiLaurentino e HeitorParadisica',
            'team': 'Calopsita',
            'icon_path': 'icon.png'
        }
